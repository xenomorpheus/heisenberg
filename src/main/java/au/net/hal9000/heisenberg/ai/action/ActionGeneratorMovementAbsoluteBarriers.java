...
public class ActionGeneratorMovementAbsoluteBarriers implements ActionGenerator {
	...
	public Path generateActions(ModelState modelState) {
		...
		if (modelState instanceof ModelStateAgentGoal) {
			...
			actions.addAll(generate(agentPos, agentPositionDelta, directionCount, bodyRadiusMax, barriers));
		}
		return actions;
	}

	/**
	 * Generate movement Action objects in the shape of a wheel spoke, using
	 * current position as centre.
	 * 
	 * @param position
	 * @param positionDelta
		...
		SpokeLoop: for (Position spoke : spokes) {
			...
			if (barriers != null) {
				for (Barrier barrier : barriers) {
					PathBlockDetails pathBlockDetails = barrier.getPathBlockDetailsDetails(position, positionNew);
