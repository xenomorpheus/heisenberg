...
public class MazeUtil {
    ...
    static Vec2[] getBoundaryShape() {
        ...
        return new Vec2[] { new Vec2(-width_half, -height_half),
     ...
     *            entity.
     * @param shape
     *            the barrier as an array of Vec2 points.
     * @param position
     *            the position of this shape.
     * @param body
     *            the object that is the barrier.
     */
    static void learnBarrierArray(MemorySet memorySet, Vec2[] shape, Vec2 position,
            Object body) {
        List<Line2D.Double> shapeLine2D = new ArrayList<>();

        for (int index = 1; index < shape.length; index++) {
            shapeLine2D.add(new Line2D.Double(shape[index - 1].x + position.x,
                    shape[index - 1].y + position.y, shape[index].x
                            + position.x, shape[index].y + position.y));
        }
