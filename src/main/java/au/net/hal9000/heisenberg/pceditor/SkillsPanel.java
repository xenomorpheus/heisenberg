...
public class SkillsPanel extends JPanel {
  ...
  public SkillsPanel(
      @NonNull Set<SkillId> skillIds, @NonNull Map<SkillId, SkillDetail> skillDetails) {
    super();
    ...
    List<SkillId> missingSkills =
        skillDetails.keySet().stream().filter(id -> !skillIds.contains(id)).toList();

    JComboBox<SkillId> skillsMissingComboBox =
        new JComboBox<>(missingSkills.toArray(new SkillId[0]));
